...
import { CommunityTag, PancakeSwapTag, DualTag } from 'components/Tags'
...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({ details, apr, multiplier, liquidity }) => {
  const pancakeSwapFarm = details

  const TranslateString = useI18n()
  const { quoteToken, token, dual } = pancakeSwapFarm
  const lpLabel = pancakeSwapFarm.lpSymbol && pancakeSwapFarm.lpSymbol.toUpperCase().replace('CAKE', 'CAKE')
  const liquidityUrlPathParts = getLiquidityUrlPathParts({
    quoteTokenAddress: quoteToken.address,
    tokenAddress: token.address,
  })
  const lpAddress = pancakeSwapFarm.lpAddresses[process.env.REACT_APP_CHAIN_ID]
  ...
  return (
    <Container>
      ...
      <ActionContainer>
        <HarvestAction {...pancakeSwapFarm} />
        <StakedAction {...pancakeSwapFarm} />
      </ActionContainer>
    </Container>
  )
}
