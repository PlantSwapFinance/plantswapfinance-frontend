...
const TokenSection: React.FC<TokenSectionProps> = ({ primaryToken, secondaryToken, children, ...props }) => {
  const renderTokenComponent = () => {
    if (!primaryToken) {
      return <SproutPlaceholderIcon width={32} mr="16px" />
    }
...
const PlantBnbTokenSection: React.FC<TokenSectionProps> = (props) => {
  return <TokenSection primaryToken={tokens.plant} secondaryToken={tokens.wbnb} {...props} />
}
  ...
  const renderTokenSection = () => {
    ...
    if (account && !hasProfile) {
      if (publicIfoData.status === 'finished') {
        return <Text textAlign="center">{t('Activate PlantSwap Profile to take part in next IFO‘s!')}</Text>
      }
      return <Text textAlign="center">{t('You need an active PlantSwap Profile to take part in an IFO!')}</Text>
    }
    ...
    if (publicIfoData.status === 'finished') {
      return userPoolCharacteristics.amountTokenCommittedInLP.isEqualTo(0) ? (
        <Flex flexDirection="column" alignItems="center">
          <SproutPlaceholderIcon width={80} mb="16px" />
