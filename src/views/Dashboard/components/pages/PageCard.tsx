...
interface PageCardProps {
  page: any
  pageId: string
  setPageListLoaded: (boolean) => void
}
...
const PageCard: React.FC<PageCardProps> = ({ page, pageId, setPageListLoaded }) => {
  const { t } = useTranslation()
  const [pageIdSelected, setPageIdSelected] = useState('')

  const handleDelete = (value) => {
    pagesApi.deletePages(value)
    setPageListLoaded(false)
  }
  ...
  const UpdateModal: React.FC<ModalProps> = ({ title, onDismiss }) => {
    ...
    const handleSubmit = (e: any) => {
      ...
      if(pageName) {
        ...
        pagesApi.updatePages(pageRef, editedPage).then(() => {
          setPageListLoaded(false)
          setPageIdSelected('')
          onDismiss()
        }).catch((err) => {
