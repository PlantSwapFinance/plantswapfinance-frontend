...
import { CommunityTag, GooseFinanceTag, DualTag } from 'components/Tags'
...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({ details, apr, multiplier, liquidity }) => {
  const gooseFarm = details

  const TranslateString = useI18n()
  const { quoteToken, token, dual } = gooseFarm
  const lpLabel = gooseFarm.lpSymbol && gooseFarm.lpSymbol.toUpperCase().replace('EGG', 'EGG')
  const liquidityUrlPathParts = getLiquidityUrlPathParts({
    quoteTokenAddress: quoteToken.address,
    tokenAddress: token.address,
  })
  const lpAddress = gooseFarm.lpAddresses[process.env.REACT_APP_CHAIN_ID]
  const bsc = `https://bscscan.com/address/${lpAddress}`
  const info = `https://pancakeswap.info/pair/${lpAddress}`
  const isCommunityFarm = gooseCommunityFarms.includes(token.symbol)

  const thisIsAToken = gooseFarm.isTokenOnly
  ...
  return (
    <Container>
      ...
      <ActionContainer>
        <HarvestAction {...gooseFarm} />
        <StakedAction {...gooseFarm} />
      </ActionContainer>
    </Container>
  )
}
