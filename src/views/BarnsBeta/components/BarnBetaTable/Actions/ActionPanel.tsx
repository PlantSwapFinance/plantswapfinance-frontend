...
const ActionPanel: React.FunctionComponent<ActionPanelProps> = ({ details, apr, multiplier, liquidity }) => {
  const barnBeta = details

  const TranslateString = useI18n()
  const { quoteToken, token, dual } = barnBeta
  const lpLabel = barnBeta.lpSymbol && barnBeta.lpSymbol.toUpperCase().replace('PLANT', 'PLANT')
  const liquidityUrlPathParts = getLiquidityUrlPathParts({
    quoteTokenAddress: quoteToken.address,
    tokenAddress: token.address,
  })
  const lpAddress = barnBeta.lpAddresses[process.env.REACT_APP_CHAIN_ID]
  const bsc = `https://bscscan.com/address/${lpAddress}`
  const info = `https://pancakeswap.info/pair/${lpAddress}`
  const isCommunityBarnBeta = communityBarnsBeta.includes(token.symbol)
  ...
  return (
    <Container>
      ...
      <ActionContainer>
        <HarvestAction {...barnBeta} />
        <StakedAction {...barnBeta} />
      </ActionContainer>
    </Container>
  )
}
