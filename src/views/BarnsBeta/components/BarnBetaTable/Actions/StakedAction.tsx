...
const Staked: React.FunctionComponent<BarnBetaWithStakedValue> = ({ pid, lpSymbol, lpAddresses, quoteToken, token }) => {
  const TranslateString = useI18n()
  const { account } = useWeb3React()
  const [requestedApproval, setRequestedApproval] = useState(false)
  const { allowance, tokenBalance, stakedBalance } = useBarnBetaUser(pid)
